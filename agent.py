...
# CRITICAL LINK RULE:
# - If the apply_link was NOT found in the original search results, write "NOT FOUND" 
# - NEVER generate URLs with IDs like /job/123456 — these are fake
# - A real link looks like: https://unstop.com/internships/ai-intern-microsoft-123456
# - If you cannot find a real verified link, set apply_link to "NOT FOUND"
...
# ── Link & Stipend Sanitizers ─────────────────────────────────────────────────
def _clean_fake_links(jobs: list[dict]) -> list[dict]:
    """Remove hallucinated apply_link values (e.g. /job/123456 style fake IDs)."""
    for job in jobs:
        link = job.get("apply_link", "") or ""
        if (
            "NOT FOUND" in link
            or re.search(r"/(?:job|jobs|careers)/\d+$", link)
            or link.endswith("123456")
            or not link.startswith("http")
        ):
            job["apply_link"] = "⚠️ Link not found — search manually on company careers page"
    return jobs
...
STIPEND/CTC RULE:
- ONLY use stipend figures explicitly mentioned in the verification data
- If not found in reviews or search results write "Not publicly available"
- NEVER invent round numbers like 40000 or 15 LPA without a source
...
""".strip()
    ...
    # Remove fake / hallucinated apply links
    jobs = _clean_fake_links(jobs)
